...
export class MultiplayerManager {
  ...
  setupRealtimeSubscription() {
    this.subscription = supabase
      .from('players')
      .on('INSERT', ({ new: rec }) => this.handlePlayerChange(rec, 'INSERT'))
      .on('UPDATE', ({ new: rec }) => this.handlePlayerChange(rec, 'UPDATE'))
      .on('DELETE', ({ old: rec }) => this.handlePlayerRemove(rec.id))
      .subscribe();
  }
