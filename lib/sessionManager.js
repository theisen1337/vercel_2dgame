...
export const saveSessionData = async (sessionId, playerData) => {
  try {
    ...
    if (error) {
      console.error('Error saving session data:', error.message, error.details, error.hint);
      return false;
    }
...
export const loadSessionData = async (sessionId) => {
  try {
    ...
    if (error) {
      console.error('Error loading session data:', error.message, error.details, error.hint);
      return null;
    }
    
    // Transform and return session data with colors object
    return {
      ...data,
      colors: {
